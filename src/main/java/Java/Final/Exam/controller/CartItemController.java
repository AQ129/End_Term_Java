...
import Java.Final.Exam.repository.AccessoryRepository;
import Java.Final.Exam.repository.CartItemRepository;
import Java.Final.Exam.repository.MemberRepository;
import Java.Final.Exam.repository.OrderItemRepository;
...
public class CartItemController {
	...
	public String updateCart(@PathVariable("id") int cartItemId, @RequestParam("quantity") int quantity, RedirectAttributes redirectAttribute) {
		...
		if (quantity <= 0) {
			...
			Accessory accessoryItem = accessoryRepo.getById(cartItem.getAccessory().getId());
			if(quantity>accessoryItem.getQuantity()){
				cartItem.setQuantity(accessoryItem.getQuantity());
			}
			else {
